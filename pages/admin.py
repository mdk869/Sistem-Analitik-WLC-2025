...
with st.form("form_tambah_peserta", clear_on_submit=True):
    ...
    umur = st.number_input("Umur", max_value=100)
    jantina = st.selectbox("Jantina", ["Lelaki", "Perempuan"])
    jabatan = st.text_input("Jabatan")
    tinggi = st.number_input("Tinggi (cm)", max_value=250)
    berat_awal = st.number_input("Berat Awal (kg)", max_value=300.0)
...
if len(data_peserta) > 0:
    ...
    berat_baru = st.number_input("Masukkan Berat Terkini (kg)", max_value=300.0)
